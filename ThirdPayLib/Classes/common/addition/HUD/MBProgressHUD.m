...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.isHiddenBackground = NO;
		self.margin = 20.0f;
		self.graceTime = 0.0f;
		self.minShowTime = 0.0f;
		self.removeFromSuperViewOnHide = NO;
        self.shouldHideOnTouch = NO;
        self.maxBoxWidth = 260.0f;
        self.boxCornerRadius = 10.0f;
...
- (void)layoutSubviews {
    ...
    maxWidth = MIN(maxWidth, self.maxBoxWidth-2*margin);
...
- (void)drawRect:(CGRect)rect {
    ...
    if (!self.isHiddenBackground) {
        ...
        float radius = self.boxCornerRadius;
        ...
        label.textColor = [UIColor colorWithRGBHex:(0x949494)];
    }
...
{
    if (self.shouldHideOnTouch && self.mode == MBProgressHUDModeText)
    {
        [self hide:YES];
    }
}
